...
{
   uint64_t source = 0x0000000000000000;
   for(int i = 7; i >= 0; i--){
      source <<= 8;
      source |= bytes[i];
   }
...
{
   if(!(high < 64 && (low <= high) && low >= 0)) return 0;
   source = source << (63 - high);
	source = source >> (64 - (high - low + 1));
...
{
   if(!(high < 64 && (low <= high) && (low >= 0))) return source;
...
{
   if(dstlow < 0 || srclow < 0 || length + dstlow > 64 || length + srclow > 64) return dest;
  
   else{    
      
      uint64_t SOUR = source;
      uint64_t DUST = dest;     

      uint64_t sBits = getBits(SOUR, srclow, srclow + (length - 1));
      sBits <<= dstlow;

      dest = clearBits(DUST, dstlow, dstlow + (length - 1));
...
{
   if(byteNum > 7 || byteNum < 0) return source;
   else{
      uint64_t mask = 0xff;
      mask <<= (byteNum * 8);
      source |= mask;
...
{
   if((!sign(op1) && !sign(op2)) && (sign(op1 + op2))){
      return true;         
   }
   if((sign(op1) && sign(op2)) && (!sign(op1 + op2))){
      return true;
...
{
   if((sign(op1) && !sign(op2)) && sign(op2 - op1)){
      return true;         
   }
   if((!sign(op1) && sign(op2)) && !sign(op2 - op1)){
      return true;
